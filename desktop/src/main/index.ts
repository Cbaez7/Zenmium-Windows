...
import { launchOpenRequests } from "./app-open";
...
app.on("second-instance", (_event, argv) => {
  for (const request of launchOpenRequests(argv)) queueOpen(request);
  if (!win) ensureWindow();
  else { if (win.isMinimized()) win.restore(); win.show(); win.focus(); }
});

// Windows passes default-browser and file-association launches in argv before
// Electron has a window. Queue them through the same validated path used for
// later second-instance requests. macOS retains its open-url/open-file events.
for (const request of launchOpenRequests(process.argv)) queueOpen(request);
