...
export function NativeSettings({ state, ui, invoke }: BrowserSurfaceProps) {
  ...
  return <>
    <h2>Zenmium</h2>
    <div className="zen-overlay-setting"><span>Default browser<small>{isDefault ? "Zenmium opens web links" : native?.defaultBrowser.packaged ? "Set Zenmium as the app for web links" : "Available in the installed Zenmium app"}</small></span><SurfaceButton disabled={disabled || isDefault || !native?.defaultBrowser.packaged} onClick={() => run("Setting default browser", NATIVE_IPC.utility, {action: "set-default-browser"})}>{isDefault ? "Default" : "Set default"}</SurfaceButton></div>
