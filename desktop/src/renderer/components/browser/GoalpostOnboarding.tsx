...
export function GoalpostOnboarding({ state, ui, invoke, close }: UtilityProps) {
  ...
  return <div className="zen-onboarding">
    ...
    {step === 4 && <section><h1>Your next page starts here.</h1><p>Web links can open in Zenmium, using your current Workspace. Your operating system keeps that choice with you.</p><SurfaceButton variant="primary" disabled={busy || !!isDefault || !native?.defaultBrowser.packaged} icon={isDefault ? Check : ExternalLink} onClick={() => run("Setting your default browser", async () => { await checkedInvoke(invoke, NATIVE_IPC.utility, { action: "set-default-browser" }); await refresh(); })}>{isDefault ? "Zenmium is your default" : "Make Zenmium the default"}</SurfaceButton>{!native?.defaultBrowser.packaged && <p className="zen-overlay-help">Default-browser setup is available from the installed Zenmium app.</p>}<div className="zen-onboarding-note"><p>Use your platform’s standard keyboard shortcuts for addresses, tabs, and commands. Hover the window gutter to reveal a hidden sidebar.</p></div></section>}
