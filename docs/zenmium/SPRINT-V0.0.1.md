...
- Chrome onboarding now discovers stable macOS and Windows Chrome profiles, derives
...
- Windows packaging now produces an x64 NSIS installer with the same ASAR and Electron fuse policy as macOS. A Windows code-signing certificate remains required for a SmartScreen-clean public release and must be supplied through the approved signing workflow, never committed to this repository.
